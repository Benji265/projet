    ...
    case 'Mine de métal':
        ...
        if ($arrayRessourceUser['metal'] >= costMetalForMineDeMetalPerLevel($_POST['buildingLevel'] + 1)) {
            if ($arrayRessourceUser['cristal'] >= costCristalForMineDeMetalPerLevel($_POST['buildingLevel'] + 1)) {
                ...
                $buildingsObj->updateBuilding(($_POST['buildingLevel'] + 1), costMetalForMineDeMetalPerLevel($_POST['buildingLevel'] + 1), costCristalForMineDeMetalPerLevel($_POST['buildingLevel'] + 1), 0, costEnergyForMineDeMetalPerLevel($_POST['buildingLevel'] + 1), $buildingsLevelTimeBuilt['Mine de métal'][$_POST['buildingLevel'] + 1], $timestamp, $_SESSION['User']['id'], $_POST['buildingName']);
